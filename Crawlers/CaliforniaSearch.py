...
# Created at: April 1, 2020
# Last updated at: April 9, 2020
...
baseUrl = "https://inmatelocator.cdcr.ca.gov/"
...
def baseCrawler(last, first):
    ...
    # accessibility popup (issue?)

    # agree and proceed to search button
    proceedButtonName = "ctl00$LocatorPublicPageContent$btnAccept"
    browser.find_element_by_name(proceedButtonName).click()


    # searching for inmate last names that start with certain character
    lastNameBar = "ctl00$LocatorPublicPageContent$txtLastName"
    browser.find_element_by_name(lastNameBar).send_keys(last)
    firstNameBar = "ctl00$LocatorPublicPageContent$txtFirstName"
    browser.find_element_by_name(firstNameBar).send_keys(first)
    browser.set_page_load_timeout(10)
    searchButton = "ctl00$LocatorPublicPageContent$btnSearch"
    ...
    while True:
        ...
        sourceSoup = BeautifulSoup(source, 'html.parser')
        rowList = sourceSoup.findAll("tr")
        for i in range(2, len(rowList) - 2):
            profileXPath = "// *[ @ id = 'ctl00_LocatorPublicPageContent_gvGridView'] / tbody / tr[" + str(i) + "] / td[1] / a"
            profile = browser.find_elements_by_xpath(profileXPath)[0]
            browser.set_page_load_timeout(10)
            profile.click()
            ...
            backButtonName = "ctl00$LocatorPublicPageContent$backButton"
            browser.find_element_by_name(backButtonName).click()
        ...
        pageXOfY = rowList[-3].text.strip().split(" ")
        if pageXOfY[1] == pageXOfY[3]:
            break
        else:
            if pageXOfY[1] == "1":
                nextButtonXPath = "//*[@id='ctl00_LocatorPublicPageContent_gvGridView']/tbody/tr[" + str(len(rowList) - 1) + "]/td/table/tbody/tr/td/a"
            else:
                nextButtonXPath = "//*[@id='ctl00_LocatorPublicPageContent_gvGridView']/tbody/tr[" + str(len(rowList) - 1) + "]/td/table/tbody/tr/td[2]/a"
            browser.find_element_by_xpath(nextButtonXPath).click()
